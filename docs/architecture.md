...
```text
                         +---------------------+
io_btn/io_dip ---------->| InputSynchronizer   |---> ButtonConditioner
                         +---------------------+              |
                                                              v
                                                     ManualInputController --+
                                                                             |
io_customIn --> InputSynchronizer --> ExternalHostInterface -----------------+--> TileCommandMux
                                                                             |          |
                                                    TileModeController <------+          v
                                                                                XorNetworkController
                                                                                 | command / response
                                                                                 v
                                                                            XingHuo_NPU Core

Manual / Host / Network RAM ports --> TileRamArbiter <--> SoC Shared RAM
Network/Core status --> DisplayController --> TileOutputAdapter --> Tile outputs
```
...
Core 内部由 `MatrixFeeder`、2×2 `SystolicArray`、`ResultCollector`、`Bias`、`Requantize`、`ReLU` 与 `ControlUnit` 组成。每个PE只保留一个本地权重寄存器；权重通道握手会原子更新完整2×2权重矩阵，忙时`weight_ready=0`。Activation（激活值）横向传播，Partial Sum（部分和）纵向传播。
