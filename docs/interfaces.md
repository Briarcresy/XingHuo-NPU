...
3. 设置 Payload/Opcode 并保持至少 3T，再翻转 Request Toggle。在 150 MHz 下为至少 20 ns。
...
BTN、DIP 和 customIn 均先由 `TileInputSynchronizer` 经过带 `ASYNC_REG` 标记的两级同步寄存器，再送入去抖或协议模块。多位总线一致性仍依赖上述稳定窗口和最终布线偏差预算；两级同步不等同于 CDC 签核。DIP 应在按键前至少稳定 3T，并保持到去抖后的操作完成。机械拨码变化后应待其稳定再按按钮。
