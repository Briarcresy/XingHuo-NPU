...
module systolic_array_2x2 (
    ...
    q8_8_mac_pe pe00 (
        ...
        .activation_in(a_left_row0), .weight_in(w_top_col0),
        .activation_out(a00_to_01), .weight_out(w00_to_10),
        .accumulator(sum00)
    );

    q8_8_mac_pe pe01 (
        .clk(clk), .rst(rst), .clear(clear), .enable(step),
        .activation_in(a00_to_01), .weight_in(w_top_col1),
        .activation_out(), .weight_out(w01_to_11),
        .accumulator(sum01)
    );

    q8_8_mac_pe pe10 (
        .clk(clk), .rst(rst), .clear(clear), .enable(step),
        .activation_in(a_left_row1), .weight_in(w00_to_10),
        .activation_out(a10_to_11), .weight_out(),
        .accumulator(sum10)
    );

    q8_8_mac_pe pe11 (
        .clk(clk), .rst(rst), .clear(clear), .enable(step),
        .activation_in(a10_to_11), .weight_in(w01_to_11),
        .activation_out(), .weight_out(),
        .accumulator(sum11)
    );
